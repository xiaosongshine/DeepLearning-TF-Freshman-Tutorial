...
with tf.Session() as sess:
    ...
    for steps in range(1000):
        ...
        if(steps % 50 == 0):
            end_time = time.time()
            print("steps is %d loss is %f  every 40 steps use tims is %f" % (steps, cross_entropy_val, end_time - start_time), end="")
            correct_prediction = tf.equal(tf.argmax(y_conv, 1), tf.argmax(y_, 1))
            accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
            accuracy_val = sess.run(accuracy, feed_dict={x: mnist.test.images[:100], y_:  mnist.test.labels[:100], keep_prob: 1.})
