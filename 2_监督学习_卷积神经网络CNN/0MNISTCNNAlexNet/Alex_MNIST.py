...
y_conv = tf.clip_by_value(y_conv, 0.00001, 1.99999)
...
with tf.Session() as sess:
    ...
    for steps in range(4000):
        ...
        cross_entropy_val, train_step_val = sess.run([cross_entropy, train_step], feed_dict={x: batch_xs, y_: batch_ys,keep_prob: 0.8})

        if(steps % 100 == 0):
            end_time = time.time()
            print("steps is %d loss is %f ,100 step use tims is %f sec" % (steps, cross_entropy_val, end_time - start_time), end="")
            #y_conv1 = AlexNet(x, False)
            correct_prediction = tf.equal(tf.argmax(y_conv, 1), tf.argmax(y_, 1))
            accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
            accuracy_val = sess.run(accuracy, feed_dict={x: mnist.test.images[:200], y_:  mnist.test.labels[:200], keep_prob: 1.})
